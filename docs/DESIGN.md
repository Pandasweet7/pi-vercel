...
浏览器  ──https──►  https://<app>.vercel.app/<path>
                        ...
                        ▼
              ┌─────────────────────────────┐
              │  Vercel Function             │   （api/proxy/index.ts + [...path].ts，Node runtime，fluid，streaming）
              │  1. Basic Auth               │
...
└─ api/proxy/
   ├─ index.ts    # 根路径 "/"（rewrite 到 /api/proxy）
   └─ [...path].ts # 其余："…" → /api/proxy/…（单括号 catch-all）
```
...
`vercel.json` 用两条 rewrite 路由：`/` → `/api/proxy`（命中 index.ts），`/((?!api/proxy/).*)` → `/api/proxy/$1`（命中 [...path].ts）。
